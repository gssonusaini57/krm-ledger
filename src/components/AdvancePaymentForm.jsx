...
const MODES = [
  ...
  { value: 'BANK_TO_MILL', label: '🏦 Bank → Mill (Expense)', hint: 'Pay a mill expense directly from bank' },
]
...
export default function AdvancePaymentForm() {
  ...
  const needsPartner  = mode !== 'BANK_TO_CASH' && mode !== 'CASH_TO_BANK' && mode !== 'BANK_TO_MILL'
  const needsCategory = mode === 'P_TO_RM' || mode === 'BANK_TO_MILL'
  ...
  const handleSubmit = () => {
    ...
    switch (mode) {
      ...
      case 'BANK_TO_MILL':
        addTransaction({
          date, amount: amt,
          description: description || 'Bank expense',
          type: TRANSACTION_TYPES.BANK_TO_MILL,
          ownerId: null, partnerId: null,
          category: category || 'EXPENDITURE',
          employeeId: employeeId || null,
          paymentMode: 'ONLINE',
  ...
  // Flow preview
  const fromLabel = (mode === 'BANK_TO_CASH' || mode === 'BANK_TO_P' || mode === 'BANK_TO_MILL') ? 'Bank'
  ...
  const toLabel = mode === 'BANK_TO_CASH' ? 'Cash'
    ...
    : mode === 'BANK_TO_MILL' ? 'Mill → Expense'
    : mode === 'P_TO_CASH' ? 'Cash'
    : mode === 'P_TO_BANK' ? 'Bank'
